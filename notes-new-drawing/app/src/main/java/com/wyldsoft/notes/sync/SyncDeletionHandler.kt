...
private const val TAG = "SyncDeletionHandler"
...
class SyncDeletionHandler(
    ...
    suspend fun applyRemoteDeletions(
        ...
        Log.d(TAG, "applyRemoteDeletions")
        try {
            ...
            for (record in manifest.deletions) {
                ...
                try {
                    when (record.entityType) {
                        "note" -> {
                            val local = noteDao.getNote(record.entityId)
                    ...
                    Log.e(TAG, "Apply deletion ${record.entityId}", e)
                    errors.add("Apply deletion ${record.entityId}: ${e.message}")
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Apply remote deletions", e)
    ...
    suspend fun updateSyncStateOnDrive(
        ...
        Log.d(TAG, "updateSyncStateOnDrive")
        try {
            ...
            Log.e(TAG, "Update sync state on Drive", e)
